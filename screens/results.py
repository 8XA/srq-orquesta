...
from modules.strings_fitting import (
    phrase_fitting,
    centered_phrase_fitting,
    colored_centered_filter
)
...
def results():
    ...
    scrapers = [
        scraper for scraper in [
            'subdivx', 'opensubtitles'
        ] if scraper in read_settings("sub_getters")
    ]
    ...
    while True:
        ...
        # Subs filtrados
        subs_filtrados = [
            sub for sub in subs if is_filtered(
                sub[0] + sub[1] + sub[2],
                plus_filter,
                minus_filter
            )
        ...
        for x in range(len(subs_pagina)):
            ...
            if subs_pagina[x][3] != 0:
                ID = (
                    colored("ID ", 'cyan', fondo, attrs=['bold', 'dark']) +
                    colored(str(subs_pagina[x][4]), 'green', fondo, attrs=['bold', 'dark'])
                )
            else:
                ID = (
                    colored("ID ", 'cyan', attrs=['bold', 'dark']) +
                    colored(str(subs_pagina[x][4]), 'green', attrs=['bold', 'dark'])
                )
        ...
        print(
            colored(
                centered_phrase_fitting(numcols, "Video a subtitular:"),
                'white',
                attrs=['bold']
            )
        )
        ...
        if not video_isfile or read_settings("selected_video_name") == "":
            msj = (
                "Cuando lo selecciones, aquí aparecerá el "
        ...
        colored_filters = colored_centered_filter(numcols, " ".join(plus_filter + minus_filter))
        ...
        i = menu(
            numcols,
            (
                f"Página: {str(pagina)} de {str(total_paginas)} | "
                f"Filtrados: {str(len(subs_filtrados))} | Total: {str(len(subs))}"
            )
        )
